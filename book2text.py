        ...
        for i, line in enumerate(split_lines):
            ...
            if line["toc"] and len(line["toc"]) > 0:
                ...
                output_path = os.path.join(
                    output_dir, f"{sequence_number}_{section_filename}.epub"
                )
...
def main(input_file, output_dir, output_csv):
    ...
    if file_type == "epub":
        success = split_epub_by_sections(
            input_file, output_dir
        )  # Changed from output_directory to output_dir
        if not success:
            print(
                "Error detected while splitting EPUB. Attempting alternative method with epubunz.py."
            )
        ...
        page_ranges = prepare_page_ranges(
            toc, regex=None, overlap=False, page_count=page_count
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Convert books to text and process them."
    )
